<template>
  <div class="row" v-if="flag === 'SUCCESS'">
    ...
    <div class="col-12"
         v-if="transaction && transaction.method.section === 'assetRegistry' && transaction.method.method === 'newLease'">
      <Lease :leaseid="leaseid" :hide-chain-details="true"/>
    </div>
    <div class="col-12"
         v-if="transaction && transaction.method.section === 'identity' && ['registerDidFor','registerDid'].includes(transaction.method.method)">
      <Identity :did="did" :hide-chain-details="true"/>
    </div>
    <div class="col-12"
         v-if="transaction && transaction.method.section === 'audits' && transaction.method.method === 'createAudit'">
      <Audit :auditid="auditid" :hide-chain-details="true"/>
    </div>
    <div class="col-12"
         v-if="transaction && transaction.method.section === 'provenance' && transaction.method.method === 'createSequence'">
          ...
          for (const event of this.transaction.events) {
            switch (event.meta.name) {
              case 'LeaseCreated':
                ...
                break;
              case 'Registered':
                let didData = event.event.data.find(d => d.id);
                if (didData) {
                  this.did = formatters.did(didData.id);
                }
                break;
              case 'AuditCreated':
